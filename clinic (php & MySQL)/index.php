...
/>
  ...
  <?php
    ...
    $sql = "SELECT * FROM appointment ORDER BY Appointment_Number DESC";  //Sort data LIFO
    ...
    if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `appointment` WHERE CONCAT(`Appointment_Number`, `Appointment_Date`, `Appointment_Time`, `Appointment_Type`, `Patient's_ID`, `Patient's_Name`, `Phone_Number`, `Doctor's_ID`, `Doctor's_Name`) LIKE '%". $valueToSearch . "%' " ;
...
<nav class="navbar fixed-top navbar-dark bg-success">
  <div class="container-fluid">
    ...
    <form class="d-flex input-group w-auto" action="index.php" method="post">
      ...
      <span class="input-group-text border-0" id="search-addon">
        ...
        <a class="nav-link" style="color: white;" href='insert.php?Appointment_Number=<?php echo $row['Appointment_Number']; ?> ?>'>Register</a>
        
        <?php if(isset($_SESSION['userlogin'])){?>
          <a class="nav-link" style="color: white;" href='logout.php'>Log Out</a><br/>
        <?php } else { ?>
          <a class="nav-link" style="color: white;" href='login.php?Appointment_Number=<?php echo $row['Appointment_Number']; ?> ?>'>Log In</a><br/>
        <?php }?>
    </form>
  </div>
</nav>
  ...
  <h1>Clinic's Listed Appointment</h1>
  <table class="table table-striped table-hover table-secondary w-50">
    ...
    <?php while($row = mysqli_fetch_array($search_result)):?>
        <tr>
          ...
          <td><a class="link" 
                 href="details.php?Appointment_Number=<?php echo $row['Appointment_Number']; ?>"
              >
              View
              </a>
